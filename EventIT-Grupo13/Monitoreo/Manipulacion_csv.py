...
def sortDatabase(csv_name):
    df = pd.read_csv(f"../Database/{csv_name}.csv")
    df.sort_values(["Participants"], axis=0, ascending=[True], inplace=True)
    return df
